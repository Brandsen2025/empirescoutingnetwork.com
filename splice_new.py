# -*- coding: utf-8 -*-
# Reusable incremental splice: build_rows.py's full ROWS (exported to
# all_rows.json by export_rows.py) is compared against what's already live in
# each platform file by pid; only rows not already present get appended. Safe
# to re-run after adding each new team's block to build_rows.py.
import json, re
...
with open('all_rows.json', encoding='utf-8') as f:
    all_rows = json.load(f)
print('all_rows.json total:', len(all_rows))
all_pids = [r['pid'] for r in all_rows]
assert len(all_pids) == len(set(all_pids)), 'duplicate pid within all_rows.json itself'
