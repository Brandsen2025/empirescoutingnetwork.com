...
assert 'loginGate' not in content, f'{fname}: login gate already present, aborting'
...
gate = gate_html(users_js, labels)
after_app = '<body>\n' + gate + '<div id="app" class="esn-hidden">'
content = content.replace(before_app, after_app, 1)

before_nav = ('<div><div class="nu-name">Oswaldo Urdiales</div><div class="nu-role">Gerente Deportivo</div></div>\n'
              '    <div class="avatar">OU</div>')
assert content.count(before_nav) == 1, f'{fname}: nav-user anchor not found once'
logout_label = labels['logout']
after_nav = (
    '<div><div class="nu-name" id="nu-name-slot">Oswaldo Urdiales</div>'
    '<div class="nu-role" id="nu-role-slot">Gerente Deportivo</div></div>\n'
    '    <div class="avatar" id="nu-avatar-slot">OU</div>\n'
...
with open(fname, 'w', encoding='utf-8') as f:
    f.write(content)
print(f'{fname}: login gate installed OK ({len(users)} users)')
