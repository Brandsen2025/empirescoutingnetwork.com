...
function mtxt(v,dec){return isPending(v)?'Sin Evaluar':v.toFixed(dec===undefined?2:dec);}
...
// convention and the empire-agents "evidencia, no calificaciones" principle.
function pct(str){
...
function renderScoutCards(){
  ...
  if(!tot){grid.innerHTML='<div class="scout-empty">Ningún jugador coincide con estos filtros. Prueba a ampliar el rango.</div>';document.getElementById('scout-more').style.display='none';return;}
  var h='';
  slice.forEach(function(p){
    ...
    h+='<div class="scout-card" onclick="openModal(\''+p.pid+'\',\''+(p.season||'')+'\',\''+(p.sq||'').replace(/\\/g,'\\\\').replace(/\'/g,"\\'")+'\')">'+
      '<div class="scout-card-top"><span class="pos-chip" style="background:'+posCol+'22;color:'+posCol+'">'+p.pos+'</span><span class="scout-card-age">'+p.age+'</span></div>'+
      ...
      '<div class="scout-card-stats">'+
        '<div><span class="scn">'+p.mp+'</span><span class="scl">PJ</span></div>'+
        '<div><span class="scn">'+p.g+'</span><span class="scl">Goles</span></div>'+
...
function footageBtns(p){
  ...
  return '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:10px">'+p.footageClips.map(function(v){
    ...
    return '<span class="t3-watch" style="opacity:.75;cursor:default;background:rgba(45,212,191,.08);border-color:rgba(45,212,191,.3);color:var(--cyan)" title="En Dropbox de ESN: '+v.dbPath+'"><i class="fas fa-film"></i>'+v.file+'</span>';
  }).join('')+'</div>';
}
function openModal(pid,season,squad){
  ...
  var html=
    '<div class="m-top">'+
      ...
      '<div>'+
        '<div class="m-name">'+p.n+(p.footageClips&&p.footageClips.length?' <i class="fas fa-film" style="color:var(--cyan);font-size:.6em;vertical-align:middle" title="Video disponible"></i>':'')+'</div>'+
        ...
        (hasProfile
          ...
          :'<span class="tm-link" style="opacity:.4;cursor:default" title="Perfil escrito aún no publicado"><i class="fas fa-clock tm-icon"></i> Profile Coming Soon</span>')+
        (p.reportPdf
    ...
    '<div class="m-body">'+
      ...
      :(p.gradingProgress!=null&&p.tier3!=null?
        '<div class="t3-partial">'+
          ...
          '<div class="t3-real-note">Confirmed sub-metrics only, shown as-is &mdash; no headline OBI/SII/composite score yet, that would overstate what\'s actually confirmado. Desglose completo en Estado de Evaluación más abajo.</div>'+
          footageBtns(p)+
        '</div>'
      :
        '<div class="t3-pitch">'+
          '<div class="t3-pitch-head"><i class="fas fa-lock"></i> Aún no evaluado por video</div>'+
          '<div class="t3-pitch-body">Las estadísticas te dicen qué pasó. No te dicen <b>por qué</b>, ni si se va a repetir. Los scouts de Empire ven partidos completos y evalúan lo que un feed de datos no puede ver físicamente:</div>'+
          '<ul class="t3-pitch-list">'+
            '<li><b>Toma de decisiones</b> bajo presión, no solo los resultados</li>'+
            '<li><b>Carácter y capacidad de aprendizaje</b> &mdash; los rasgos que definen el éxito o fracaso de un fichaje</li>'+
            '<li><b>Inteligencia sin balón</b> &mdash; anticipación, ocupación de espacios, movimientos que ninguna estadística muestra</li>'+
            '<li><b>Evidencia con marca de tiempo</b> para cada calificación &mdash; ningún número sin un clip que lo respalde</li>'+
          '</ul>'+
          '<div class="t3-pitch-note">Esto es lo que distingue a Empire FGA de una plataforma de estadísticas. Se desbloquea en el momento en que este jugador es evaluado.</div>'+
          footageBtns(p)+
        '</div>'
      ))+
      '<div class="m-sec"><i class="fas fa-clipboard-check" style="margin-right:5px;opacity:.6"></i>Grading Status</div>'+
      '<div class="status-grid">'+
        '<div class="status-row"><span class="status-lbl">Datos de Temporada</span><span class="status-val" style="color:#5FA87A">Completo</span></div>'+
        '<div class="status-row"><span class="status-lbl">Análisis de Video</span><span class="status-val" style="color:'+(p.gradingProgress>=12?'#5FA87A':(p.gradingProgress>=1?'#C9A961':'var(--t3)'))+'">'+(p.gradingProgress!=null?p.gradingProgress+' de 12 métricas':'Aún no iniciado')+'</span></div>'+
        '<div class="status-row"><span class="status-lbl">Calificación Compuesta</span><span class="status-val" style="color:'+(isPending(fga)?'var(--t3)':'#5FA87A')+'">'+(isPending(fga)?'Pending':'Complete')+'</span></div>'+
      '</div>'
    '</div>';
...
function wageBadge(p,compact){
  ...
  var map = {
    ...
    unconfirmed: {label:'Nivel salarial sin confirmar', short:'?', col:'#5A6472'}
  };
...
function addTgt(btn,key){
  ...
  if(tgts.length>=20){alert('Máximo 20 objetivos');return;}
...
function renderTargets(){
  ...
  if(!tgts.length){
    g.innerHTML='<div class="empty" style="grid-column:1/-1"><div class="empty-i"><i class="fas fa-crosshairs"></i></div><div class="empty-m">Sin Objetivos</div><div class="empty-s">Haz clic en + en cualquier jugador para iniciar tu lista de seguimiento</div></div>';
...
function renderCmp(){
  ...
  var h='<thead><tr><th class="rh">Métrica</th>';
...
function drawCharts(){
  ...
  if(el2){
    var bands=[{r:'80–100',c:FB['80-100']||0,col:'#EDB84E',l:'Elite'},{r:'60–80',c:FB['60-80']||0,col:'#22D46A',l:'Excellent'},{r:'40–60',c:FB['40-60']||0,col:'#2DD4BF',l:'Muy Bueno'},{r:'20–40',c:FB['20-40']||0,col:'#4A7CC4',l:'Standard'},{r:'0–20',c:FB['0-20']||0,col:'#2A3A4E',l:'Developing'},{r:'—',c:FB['PENDING']||0,col:'#5A6472',l:'Evaluación Pendiente'}];
...
function drawRadar(){
  ...
  el.innerHTML=sampleN>0?svgRadar(axes,210):'<div class="empty" style="padding:20px 0"><div class="empty-m" style="font-size:.7rem">Sin jugadores evaluados'+(lg?' en '+lg:'')+'</div></div>';
  var note=document.getElementById('ch-radar-note');
  if(note)note.textContent=sampleN>0?('Promedio de '+sampleN.toLocaleString()+' jugadores evaluados'+(lg?' en '+lg:' · platform-wide')+' · 0\u2013100 scale'):'';
}
function drawSch(){
...
function renderMetrics(){
  ...
  var tiers=[
    {k:'T1',lbl:'Nivel 1 — Métricas Base',sub:'21 metrics · 60% Automated · FBref + Sofascore',cls:'t1c',span:2},
    {k:'T2',lbl:'Nivel 2 — Inteligencia Compuesta',sub:'6 metrics · 30% Semi-Automated · Multi-factor',cls:'t2c',span:1},
    {k:'T3',lbl:'Nivel 3 — Inteligencia de Scout',sub:'11 metrics · 10% Manual · Expert assessment',cls:'t3c',span:1},
  ];
...
var HD_LBL={"total": "Total de Jugadores", "leagues": "Ligas", "countries": "Países", "continents": "Continentes", "players": "jugadores", "league1": "liga", "leagueN": "ligas", "countryWord": "países", "leagueWord": "ligas"};
var HD_CNAMES={"Europe": "Europa", "South America": "Sudamérica", "North America": "Norteamérica", "Asia": "Asia", "Africa": "África", "Oceania": "Oceanía", "Other": "Otro"};
